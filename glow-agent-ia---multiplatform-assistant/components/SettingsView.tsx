...
const SettingsView: React.FC<SettingsViewProps> = ({ darkMode, toggleDarkMode, language, setLanguage }) => {
  ...
  const translations: any = {
    es: {
      title: 'Configuración',
      subtitle: 'Personaliza tu experiencia en Glow Agent IA.',
      appearance: 'Apariencia',
      darkMode: 'Modo Oscuro',
      darkModeDesc: darkMode ? 'Tema Oscuro Activado' : 'Tema Claro Activado',
      langRegion: 'Idioma y Región',
      interfaceLang: 'Idioma de la Interfaz',
      timezone: 'Zona Horaria del Sistema',
      timezoneDesc: 'Detectado automáticamente desde tu equipo',
    ...
    en: {
      title: 'Settings',
      subtitle: 'Customize your Glow Agent IA experience.',
      appearance: 'Appearance',
      darkMode: 'Dark Mode',
      darkModeDesc: darkMode ? 'Dark Theme Enabled' : 'Light Theme Enabled',
      langRegion: 'Language & Region',
      interfaceLang: 'Interface Language',
      timezone: 'System Timezone',
      timezoneDesc: 'Automatically detected from your device',
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full h-full overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        <h2 className="text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">{t.title}</h2>
      ...
      <div className="space-y-6 pb-20">
        ...
        {/* Apariencia */}
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm transition-colors duration-500">
           ...
           <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-slate-800">
              ...
              <button 
                ...
                className={`w-14 h-8 rounded-full p-1 transition-all duration-300 flex items-center ${darkMode ? 'bg-indigo-600 justify-end' : 'bg-slate-200 dark:bg-slate-700 justify-start'}`}
              >
                 <div className="w-6 h-6 bg-white dark:bg-slate-200 rounded-full shadow-md flex items-center justify-center">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                 ...
                 <select 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors"
                 >
              ...
              <div>
                 ...
                 <div className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm text-slate-600 dark:text-slate-300 flex items-center gap-3 transition-colors">
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm transition-colors duration-500">
           ...
           <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl flex justify-between items-center transition-colors">
              <div>
              ...
              <button onClick={() => window.aistudio?.openSelectKey()} className="text-xs bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-3 py-2 rounded-lg font-bold hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-white transition-all">
                 {t.changeKey}
              </button>
           </div>
           <div className="mt-8 text-xs text-slate-400 text-center">
              Glow Agent IA • Versión 2.5.0 Stable
           </div>
        </section>
